...
function DropdownMenuItem({
  ...
  return (
    <DropdownMenuPrimitive.Item
      ...
      className={cn(
        "group/dropdown-menu-item relative flex cursor-default items-center gap-2.5 rounded-sm px-3 py-2 text-xs font-medium tracking-wider uppercase outline-hidden select-none focus:bg-zinc-100 focus:text-zinc-950 data-highlighted:bg-zinc-100 data-highlighted:text-zinc-950 data-inset:pl-9.5 data-[variant=destructive]:text-destructive data-[variant=destructive]:focus:bg-destructive/10 data-[variant=destructive]:focus:text-destructive data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-3.5 data-[variant=destructive]:*:[svg]:text-destructive",
        className
      )}
      {...props}
    />
...
function DropdownMenuSubContent({
  ...
  return (
    <DropdownMenuPrimitive.SubContent
      ...
      className={cn("z-50 min-w-36 origin-(--radix-dropdown-menu-content-transform-origin) overflow-hidden rounded-md border bg-white p-1.5 text-zinc-950 shadow-xl ring-1 ring-black/10 duration-100 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95", className )}
      style={{ zIndex: 1100, backgroundColor: '#ffffff', color: '#111111', ...style }}
      {...props}
    />
  )
