...
function PaginationPrevious({ className, ...props }: React.ComponentProps<typeof Button>) {
  return (
    <Button aria-label="Go to previous page" size="sm" variant="ghost" className={cn("gap-1 px-2.5", className)} {...props}>
      <ChevronLeftIcon />
      <span>Назад</span>
    </Button>
  )
...
function PaginationNext({ className, ...props }: React.ComponentProps<typeof Button>) {
  return (
    <Button aria-label="Go to next page" size="sm" variant="ghost" className={cn("gap-1 px-2.5", className)} {...props}>
      <span>Вперед</span>
      <ChevronRightIcon />
    </Button>
  )
